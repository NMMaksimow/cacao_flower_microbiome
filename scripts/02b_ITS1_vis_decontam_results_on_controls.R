...
ps_ITS1_otu97_raw      <- readRDS(here("results", "rds", "ps_ITS1_otu97_raw.rds"))
ps_ITS1_otu97_decontam <- readRDS(here("results", "rds", "ps_ITS1_otu97_decontam.rds"))
...
# ── Expected mock community taxa (Family | Genus, UNITE taxonomy) ─────────────
# Used to assign taxa to the "Mock community" legend group.
# Notes:
#   - Cryptococcus neoformans family verified from tax_table(ps_ITS1_fungi).
#   - Saccharomyces cerevisiae: ITS1 amplicon too long for paired-end merging →
#     not expected to be detected, but included to document method limitation.
mock_expected_ITS1 <- c(
        "f__Cryptococcaceae | g__Cryptococcus",
        "f__Saccharomycetaceae | g__Saccharomyces"
)

# ── Long-table helper ────────────────────────────────────────────────────────
# Melts a phyloseq object (control samples only) to a tidy table.
# Species level is omitted: colour coding at Family | Genus level is sufficient
# to show that contamination originates from a small number of ASVs.
...
control_long_tbl <- bind_rows(
        make_control_long_table(ps_ITS1_otu97_raw,      "Before decontam"),
        make_control_long_table(ps_ITS1_otu97_decontam, "After decontam"),
        make_control_long_table(ps_ITS1_otu97_fungi,    "After off-target removal")
) |>
        mutate(stage = factor(stage, levels = c("Before decontam",
                                                "After decontam",
                                                "After off-target removal")))

# ── Union threshold: taxa with mean within-sample RA > 1 % in any condition ──
# IMPORTANT: OTUs must be aggregated to Family_Genus level BEFORE computing RA.
# Without this step, a genus split across multiple OTUs would underestimate its
# total RA (averaging per-OTU RAs instead of summing them), causing mock
# community taxa to be collapsed to "Other" while single-OTU contaminants pass.
RA_THRESHOLD <- 0.01
...
mock_in_union   <- intersect(mock_expected_ITS1, union_taxa)
contam_in_union <- setdiff(union_taxa, mock_expected_ITS1)
...
raw_sums   <- sample_sums(ps_ITS1_otu97_raw)
final_sums <- sample_sums(ps_ITS1_otu97_fungi)
...
ggsave(here("results", "figures", "02b_ITS1_otu97_controls_pipeline_stages.png"),
       fig_control_bars_ITS1, width = 16, height = 11, dpi = 300)
