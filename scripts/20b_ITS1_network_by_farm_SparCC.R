## ============================================================================
## 20b — Co-occurrence networks by farm: ITS1 OTU97 fungi (bagged vs unbagged) — SparCC
##
## SparCC alternative to 19b (SpiecEasi-MB). Identical logic to 20a (16S).
...
## Output:
##   results/rds/20b_network_results.rds
##   results/figures/20b_network_panels.png
##   results/tables/20b_network_topology.csv
##   results/tables/20b_network_hubs.csv
## ============================================================================
...
CLASS_NA_COLOUR <- "grey70"    # colour for nodes with no phylum assignment
...
plot_net_panel <- function(farm_result, which_net, farm, treatment_label,
        ...
        igraph::V(g)$class     <- tt_raw[igraph::V(g)$name, "Class"]
                ...
                return(
                        ggraph(g, layout = "circle") +
                                ...
                                scale_colour_manual(values = cls_palette, name = "Class",
                                                    na.value = CLASS_NA_COLOUR, guide = "none") +
...
# Build a consistent class colour palette from all genera retained globally.
# The same named vector is passed to every plot_net_panel() call so class
...
all_cls  <- sort(unique(na.omit(as.character(tt_bvu[, "Class"]))))
n_cls    <- length(all_cls)
cls_cols <- setNames(ggsci::pal_d3("category20")(n_cls), all_cls)
cat(sprintf("  Classes in network: %d\n", n_cls))
...
saveRDS(results, here("results", "rds", "20b_network_results.rds"))
cat("  Saved: 20b_network_results.rds\n")
...
if (!exists("results")) {
        results <- readRDS(here("results", "rds", "20b_network_results.rds"))
        cat("  Loaded: 20b_network_results.rds\n")
}
if (!exists("ps_bvu") || !exists("cls_cols")) {
        ps_bvu <- ps_raw |>
                subset_samples(sample_type %in% c("bagged_flower", "unbagged_flower")) |>
                (\(x) prune_samples(sample_sums(x) >= MIN_LIB, x))() |>
                (\(x) prune_taxa(taxa_sums(x) > 0, x))()
        ...
        all_cls  <- sort(unique(na.omit(as.character(tt_vis[, "Class"]))))
        n_cls    <- length(all_cls)
        cls_cols <- setNames(ggsci::pal_d3("category20")(n_cls), all_cls)
        cat(sprintf("  Classes: %d\n", n_cls))
}

title_str   <- "ITS1 OTU97 Fungi — SparCC co-occurrence networks by farm (bagged vs unbagged)"
caption_str <- sprintf(
...
ggsave(
        here("results", "figures", "20b_network_panels_sparcc.png"),
        plot   = fig,
        width  = 24,
        height = 14,
        dpi    = 300
)
cat("  Saved: 20b_network_panels_sparcc.png\n")
...
if (any(!sapply(topology_rows, is.null))) {
        dplyr::bind_rows(topology_rows) |>
                ...
                readr::write_csv(here("results", "tables", "20b_network_topology.csv"))
        cat("  Saved: 20b_network_topology.csv\n")
} else {
...
if (nrow(hub_combined) > 0 && "treatment" %in% names(hub_combined)) {
        hub_combined |>
                ...
                readr::write_csv(here("results", "tables", "20b_network_hubs.csv"))
        cat("  Saved: 20b_network_hubs.csv\n")
} else {
...
if (nrow(cmp_combined) > 0) {
        cmp_combined |>
                ...
                readr::write_csv(here("results", "tables", "20b_network_comparison.csv"))
        cat("  Saved: 20b_network_comparison.csv\n")
} else {
        cat("  No comparison data (netCompare skipped for all farms).\n")
}

n_success <- sum(!sapply(results, is.null))
cat(sprintf("\n20b done. %d / %d farms analysed successfully.\n",
