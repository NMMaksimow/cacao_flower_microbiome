# ---- 0. Parameters -------------------------------------------------------
# Distance metrics in canonical order (matches 20a_16S_pcoa_by_farms.R)
...
# ---- 2. Load data --------------------------------------------------------
dist_list <- readRDS(here("results", "rds", "19a_16S_dist_list.rds"))
meta_list <- readRDS(here("results", "rds", "19a_16S_meta_list.rds"))

missing_dist <- setdiff(DIST_LEVELS, names(dist_list))
if (length(missing_dist) > 0) {
  stop(sprintf(
    "dist_list missing: %s — re-run 19a_16S_beta_perm.R",
...
# Figure: 6 distances × 6 colour variables
...
# Same NDC approach as 20a_16S_pcoa_by_farms.R
omi_h   <- par("omi")
din_h   <- par("din")[2]
