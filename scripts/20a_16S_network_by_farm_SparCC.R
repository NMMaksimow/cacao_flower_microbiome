## ============================================================================
## 20a — Co-occurrence networks by farm: 16S bacteria (bagged vs unbagged) — SparCC
##
## SparCC alternative to 19a (SpiecEasi-MB). SparCC estimates pairwise
## log-ratio correlations between taxa via iterative compositional component
## removal. It does not use STARS lambda selection and has no p >> n constraint,
## making it suitable as a diagnostic when SpiecEasi returns empty networks.
##
## Differences from 19a:
##   - measure = "sparcc"; normMethod = "TSS"; zeroMethod = "pseudo"
##   - sparsMethod = "threshold" (keep |r| > SPARCC_THRESH, not STARS)
##   - No MAX_TAXA_NET cap (SparCC is not affected by large p)
##   - permTest = FALSE (SparCC permutations are ~5x slower; enable for publication)
##   - Edges represent co-abundance correlations, NOT partial correlations
##
## Biological note: SparCC edges include indirect associations (taxon A and C
## both correlate with B → appear connected). SpiecEasi removes these; SparCC
## does not. Networks here are therefore denser and less specific but serve as
## a diagnostic for the presence of any co-occurrence structure.
##
## Input:
##   results/rds/ps_16S_bacteria_biosamples.rds
## Output:
##   results/rds/20a_network_results.rds
##   results/figures/20a_network_panels.png
##   results/tables/20a_network_topology.csv
##   results/tables/20a_network_hubs.csv
## ============================================================================

# ── 0. Parameters ─────────────────────────────────────────────────────────────

PREV_THRESH_NET <- 0.20    # 20% per group; ≥4–5 of 21 samples
SPARCC_THRESH   <- 0.5     # |r| edge inclusion cutoff (standard SparCC default)
...
cat("\n── 4. Per-farm networks (SparCC via NetComi) ────────────────────────────\n")
...
for (farm in FARM_LEVELS) {
        ...
        # No taxa cap: SparCC does not use LASSO regression, so p >> n is not a problem.
        ...
        # SparCC: works on proportions (normMethod = "TSS"); pseudo handles log(0).
        # sparsMethod = "threshold" keeps |r| > SPARCC_THRESH as edges (no STARS selection).
        # permTest = FALSE: SparCC permutations are very slow; enable for publication.
        net_raw <- tryCatch(
                netConstruct(
                        data        = ps_bag,
                        data2       = ps_unb,
                        ...
                        normMethod  = "TSS",       # counts → proportions
                        zeroMethod  = "pseudo",    # +1 pseudocount before log transform
                        sparsMethod = "threshold", # keep |r| > SPARCC_THRESH
                        thresh      = SPARCC_THRESH,
                        verbose     = 0
                ),
                error = function(e) {
...
saveRDS(results, here("results", "rds", "20a_network_results.rds"))
cat("  Saved: 20a_network_results.rds\n")
...
if (!exists("results")) {
        results <- readRDS(here("results", "rds", "20a_network_results.rds"))
        cat("  Loaded: 20a_network_results.rds\n")
}
...
title_str   <- "16S Bacteria — SparCC co-occurrence networks by farm (bagged vs unbagged)"
caption_str <- sprintf(
        "SparCC  |  prevalence >= %.0f%%  |  |r| >= %.2f edge threshold  |  hub nodes = black border",
        100 * PREV_THRESH_NET, SPARCC_THRESH
)
...
ggsave(
        here("results", "figures", "20a_network_panels_sparcc.png"),
        plot   = fig,
        width  = 24,
        height = 14,
        dpi    = 300
)
cat("  Saved: 20a_network_panels_sparcc.png\n")
...
if (any(!sapply(topology_rows, is.null))) {
        dplyr::bind_rows(topology_rows) |>
                ...
                readr::write_csv(here("results", "tables", "20a_network_topology.csv"))
        cat("  Saved: 20a_network_topology.csv\n")
} else {
...
if (nrow(hub_combined) > 0 && "treatment" %in% names(hub_combined)) {
        hub_combined |>
                ...
                readr::write_csv(here("results", "tables", "20a_network_hubs.csv"))
        cat("  Saved: 20a_network_hubs.csv\n")
} else {
...
if (nrow(cmp_combined) > 0) {
        cmp_combined |>
                ...
                readr::write_csv(here("results", "tables", "20a_network_comparison.csv"))
        cat("  Saved: 20a_network_comparison.csv\n")
} else {
        cat("  No comparison data (netCompare skipped for all farms).\n")
}

n_success <- sum(!sapply(results, is.null))
cat(sprintf("\n20a done. %d / %d farms analysed successfully.\n",
