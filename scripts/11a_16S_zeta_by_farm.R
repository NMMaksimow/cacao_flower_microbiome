...
# ---- 2. Load data --------------------------------------------------------
# No prevalence filter before zeta: presence/absence method; rare taxa contribute
...
# Incidence matrix: samples × ASVs (0/1)
incidence <- ps |>
  otu_table() |>
  as("matrix") |>
...
# For each farm × treatment: Zeta.decline.mc() draws SAM random subsets of
# increasing size (orders 1:ZETA_ORDERS) and computes the mean number of shared
# ASVs. All ZETA_ORDERS computed in a single call (faster than order-by-order).
# Results stored in a nested list: zeta_results[[farm]][[group]].
...
for (farm in FARM_LEVELS) {
  ...
  for (grp in c("bagged_flower", "unbagged_flower")) {
    ...
    # Trim ASV columns absent from this farm-group subset before passing to
    # Zeta.decline.mc() — avoids operating on a 21 × 25k sparse matrix.
...
saveRDS(zeta_results,
        here("results", "rds", "zeta_16S_by_farm.rds"),
        compress = "xz")
...
# AIC winner characterises community assembly mechanism per farm × treatment.
...
write_csv(model_fits,
          here("results", "tables", "11a_16S_zeta_by_farm_model_fits.csv"))

cat("Saved: results/tables/11a_16S_zeta_by_farm_model_fits.csv\n")
cat("Done.\n")
