## ============================================================================
## 13a — Indicator Species Analysis (ISA) — 16S Bacteria (CFM)
##
## Presence/absence ISA (IndVal.g) on 16S ASV data.
## Contrast: bagged vs unbagged flowers — insect-vectored bacteria
##
...
## Output:
##   results/figures/13a_ISA_insect_footprint.png
##   results/tables/13a_ISA_insect_footprint.csv
## ============================================================================
...
plot_indicator_heatmap_with_farms <- function(
        ...
        rank          = RANK_PLOT,
        min_asv       = 1,
        feature_label = "ASVs",   # "ASVs" for 16S, "OTUs" for ITS1
        title         = NULL,
        show_x_labels = TRUE,
        ...
        # ── Genus-level aggregation (preserves ordering and label from original) ──
        ...
        hits_genus <- hits |>
                ...
                dplyr::mutate(
                        Taxon_label = {
                                val <- !!rank_sym
                                # fall back to higher ranks if genus is uninformative
...
p_unbagged <- plot_indicator_heatmap_with_farms(
        ...
        title         = "Unbagged-flower indicators (insect-vectored bacteria)",
        show_x_labels = FALSE,
        feature_label = "ASVs"
)
...
fig <- (p_unbagged / p_bagged) +
        plot_layout(heights = c(max(2, n_rows_ub), max(2, n_rows_b)),
                    guides  = "collect") +
        plot_annotation(
                title   = "ISA: Bagged vs unbagged flowers — 16S bacteria",
                caption = sprintf(
...
ggsave(
        here("results", "figures", "13a_ISA_insect_footprint.png"),
        plot   = fig,
        width  = 12,
        height = max(5, (n_rows_ub + n_rows_b) * 0.6 + 3),
        dpi    = 300
)
cat("  Saved: 13a_ISA_insect_footprint.png\n")
...
if (nrow(hits_bvu) > 0) {
        hits_bvu |>
                ...
                readr::write_csv(here("results", "tables",
                                      "13a_ISA_insect_footprint.csv"))
        cat("  Saved: 13a_ISA_insect_footprint.csv\n")
}
