## ============================================================================
## 14a — Indicator Species Analysis (ISA) — 16S Bacteria (CFM)
##        Pollinator footprint: unpollinated / low-poll / high-poll
##
## Contrast: unbagged flowers only (pi_clsm not NA).
##   unpollinated — pi_clsm == 0       (n = 38)
##   low_poll     — pi_clsm 1–39       (n = 49)
##   high_poll    — pi_clsm >= PI_THRESH (n = 15)
##
## Three-group ISA using IndVal.g. IndVal.g normalises specificity by
## within-group means, so it is already robust to the 38/49/15 imbalance;
## no downsampling is needed.
##
## Farm-confounding test flags any indicator taxon where >= FARM_CONC of
## all detections come from a single farm.
##
## Input:
##   results/rds/ps_16S_bacteria_biosamples.rds
## Output:
##   results/figures/14a_ISA_pollinator_footprint.png
##   results/tables/14a_ISA_pollinator_footprint.csv
## ============================================================================
...
PI_THRESH   <- 40     # pi_clsm cut-off for high-pollination class
...
extract_isa_hits <- function(mp_result, physeq, group_var,
        ...
        hits <- sign_df |>
                ...
                dplyr::mutate(
                        # index 1/2/3 → single-group indicators; >3 → combinations
...
# panel widths proportional to number of columns (n_groups : 7 ratio).
...
#            ("sample_type" for 13a/13b, "pollination_class" for 14a/14b)
...
plot_indicator_heatmap_with_farms <- function(
        ...
        # Pre-compute column name vectors for ordering
        focal_prev_col  <- paste0("prev_", indicator_grp)
        other_prev_cols <- paste0("prev_", setdiff(group_cols, indicator_grp))
        ...
        hits_genus <- hits |>
                ...
                # ordering: focal prevalence minus mean of others (ascending → most indicative at top)
                dplyr::mutate(
                        ordering = .data[[focal_prev_col]] -
                                rowMeans(dplyr::pick(dplyr::all_of(other_prev_cols)),
                                         na.rm = TRUE)
                ) |>
                ...
                dplyr::mutate(
                        Taxon_label = {
                                val <- !!rank_sym
                                if (!is.na(val) && nchar(as.character(val)) > 3)
                                        as.character(val)
                                else if (!is.na(Order) && nchar(as.character(Order)) > 3)
                                        as.character(Order)
                                else if (!is.na(Class) && nchar(as.character(Class)) > 3)
                                        as.character(Class)
                                else if (!is.na(Phylum) && nchar(as.character(Phylum)) > 3)
                                        as.character(Phylum)
        ...
        if (length(present_asvs) > 0) {
                farm_raw <- as.data.frame(t(pa[present_asvs, , drop = FALSE])) |>
                        ...
                        # collapse: a sample is positive for a genus if any ASV detected
        ...
        # Build custom x-axis labels: add "(n=N)" to treatment columns if requested
        x_labels <- if (!is.null(n_per_group)) {
                c(
                        ...
                        setNames(farm_levels, farm_levels)  # farm labels unchanged
                )
        } else NULL
...
cat("\n── 2. Pollinator footprint ISA (3 groups) ───────────────────────────────\n")

sdata <- as(sample_data(ps_16S), "data.frame")
keep  <- sdata$sample_type == "unbagged_flower" & !is.na(sdata$pi_clsm)
ps_poll <- prune_samples(keep, ps_16S)

sample_data(ps_poll)$pollination_class <- factor(
        dplyr::case_when(
                sample_data(ps_poll)$pi_clsm == 0              ~ "unpollinated",
                sample_data(ps_poll)$pi_clsm <  PI_THRESH      ~ "low_poll",
                TRUE                                             ~ "high_poll"
        ),
        levels = c("unpollinated", "low_poll", "high_poll")
)

n_per_group <- table(sample_data(ps_poll)$pollination_class) |>
        as.integer() |>
        setNames(levels(sample_data(ps_poll)$pollination_class))

cat(sprintf("  Groups: unpollinated n=%d, low_poll n=%d, high_poll n=%d (PI >= %d)\n",
            n_per_group["unpollinated"], n_per_group["low_poll"],
            n_per_group["high_poll"], PI_THRESH))

ps_bvu <- ps_poll |> prune_for_isa()
...
mp_bvu <- run_isa(ps_bvu, "pollination_class")

hits_bvu <- extract_isa_hits(mp_bvu, ps_bvu, "pollination_class")
if (nrow(hits_bvu) > 0) {
        conf_bvu <- test_farm_confounding(hits_bvu, ps_bvu)
        hits_bvu <- dplyr::left_join(hits_bvu, conf_bvu, by = "ASV")

        cat(sprintf(
                "  Indicators: %d total  (unpollinated: %d, low_poll: %d, high_poll: %d, combination: %d)  |  %d farm-confounded\n",
                nrow(hits_bvu),
                sum(hits_bvu$indicator_group == "unpollinated"),
                sum(hits_bvu$indicator_group == "low_poll"),
                sum(hits_bvu$indicator_group == "high_poll"),
                sum(hits_bvu$indicator_group == "combination"),
...
grp_cols_bvu <- c("unpollinated", "low_poll", "high_poll")

# Top panel: x-labels suppressed
p_high <- plot_indicator_heatmap_with_farms(
        hits_bvu, ps_bvu, "high_poll", grp_cols_bvu,
        title         = paste0("High-pollination indicators (PI >= ", PI_THRESH, ")"),
        show_x_labels = FALSE,
        feature_label = "ASVs",
        n_per_group   = n_per_group
)

# Middle panel: x-labels suppressed
p_low <- plot_indicator_heatmap_with_farms(
        hits_bvu, ps_bvu, "low_poll", grp_cols_bvu,
        title         = "Low-pollination indicators (PI 1-39)",
        show_x_labels = FALSE,
        feature_label = "ASVs",
        n_per_group   = n_per_group
)

# Bottom panel: x-labels visible
p_unp <- plot_indicator_heatmap_with_farms(
        hits_bvu, ps_bvu, "unpollinated", grp_cols_bvu,
        title         = "Unpollinated indicators (PI = 0)",
        show_x_labels = TRUE,
        feature_label = "ASVs",
        n_per_group   = n_per_group
)
...
n_hp  <- n_rows("high_poll")
n_lp  <- n_rows("low_poll")
n_unp <- n_rows("unpollinated")

fig <- (p_high / p_low / p_unp) +
        plot_layout(heights = c(max(2, n_hp), max(2, n_lp), max(2, n_unp)),
                    guides  = "collect") +
        plot_annotation(
                title   = paste0("ISA: Pollinator footprint — 16S bacteria",
                                 "  (unbagged flowers, PI groups)"),
                caption = sprintf(
...
ggsave(
        here("results", "figures", "14a_ISA_pollinator_footprint.png"),
        plot   = fig,
        width  = 12,
        height = max(6, (n_hp + n_lp + n_unp) * 0.6 + 3),
        dpi    = 300
)
cat("  Saved: 14a_ISA_pollinator_footprint.png\n")
...
if (nrow(hits_bvu) > 0) {
        hits_bvu |>
                ...
                readr::write_csv(here("results", "tables",
                                      "14a_ISA_pollinator_footprint.csv"))
        cat("  Saved: 14a_ISA_pollinator_footprint.csv\n")
}

cat(sprintf("\n14a done. %d indicators found.\n", nrow(hits_bvu)))
