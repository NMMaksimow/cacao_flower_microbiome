# ---- 0. Parameters -------------------------------------------------------
PREV_THRESHOLD <- 0.10   # must match 08a (prevalence filter to reproduce same samples)
...
MGMT           <- c(ib = "inside_forest", vr = "near_forest",  sa = "near_forest",
                    ...
                    yb = "full_sun")
# Colours match Python notebook (notebooks/01a_16s_pca_by_farm.ipynb)
...
if (length(missing_keys) > 0) {
  stop(sprintf(
    "dist_list missing: %s — re-run 08a_16S_pcoa_global.R to regenerate the RDS.",
...
if (length(missing_meta) > 0) {
  stop(sprintf(
    "meta_list missing: %s — re-run 08a_16S_pcoa_global.R to regenerate the RDS.",
    paste(missing_meta, collapse = ", ")
  ))
}

# ---- 3. Within-farm PCoA (shared axis limits within each distance row) ---
# Layout: rows = 6 distances, columns = 7 farms (42 panels total).
...
# Aitchison distances use all samples; rarefied distances (BC, Jaccard, UniFrac)
# use only samples that survived rarefaction — panel n may differ from first row.

dir.create(here("results", "figures"), showWarnings = FALSE, recursive = TRUE)

png(here("results", "figures", "09a_16S_pcoa_per_farm_shared_lim.png"),
    width = 20, height = 16, units = "in", res = 300)

# mfrow: 6 distance rows × 7 farm columns
...
for (dname in DIST_LEVELS) {
  ...
  for (farm in FARM_LEVELS) {
    ...
    # mar_top: tight top margin for rows 2–6 since they have no panel title.
    ...
    # Farm + management title only for the top row; empty string for rows 2–6
...
mtext("Within-farm PCoA — 16S ASV bacteria (shared axis limits within distance rows)",
...
message("Done. PCoA saved to results/figures/09a_16S_pcoa_per_farm_shared_lim.png")
