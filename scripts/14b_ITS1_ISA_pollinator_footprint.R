## ============================================================================
## 14b — Indicator Species Analysis (ISA) — ITS1 Fungi (CFM)
...
## Output:
##   results/figures/14b_ISA_pollinator_footprint.png
##   results/tables/14b_ISA_pollinator_footprint.csv
## ============================================================================
...
ps_poll <- prune_samples(keep, ps_ITS1)
...
fig <- (p_high / p_low / p_unp) +
        plot_layout(heights = c(max(2, n_hp), max(2, n_lp), max(2, n_unp)),
                    guides  = "collect") +
        plot_annotation(
                title   = paste0("ISA: Pollinator footprint — ITS1 OTU97 fungi",
...
ggsave(
        here("results", "figures", "14b_ISA_pollinator_footprint.png"),
        plot   = fig,
        width  = 12,
        height = max(6, (n_hp + n_lp + n_unp) * 0.6 + 3),
        dpi    = 300
)
cat("  Saved: 14b_ISA_pollinator_footprint.png\n")
...
if (nrow(hits_bvu) > 0) {
        hits_bvu |>
                ...
                readr::write_csv(here("results", "tables",
                                      "14b_ISA_pollinator_footprint.csv"))
        cat("  Saved: 14b_ISA_pollinator_footprint.csv\n")
}

cat(sprintf("\n14b done. %d indicators found.\n", nrow(hits_bvu)))
