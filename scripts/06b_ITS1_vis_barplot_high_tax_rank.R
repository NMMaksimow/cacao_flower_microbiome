## ============================================================================
## 06b — ITS1 Phylum-level stacked barplots (CFM)
##
...
##
## Key fix vs 06b_ (prev. 07b_): "Other" is now summed within each sample before any
## cross-sample averaging, so stacks correctly reach 100%.
##
## Input:  results/rds/ps_ITS1_otu97_fungi_biosamples.rds   (main)
## Output: results/figures/06b_ITS1_barplot_phylum_*.png
## ============================================================================
...
RA_THRESHOLD_PHYLUM <- 0.005    # Phyla with mean RA < 1% → "Other"

MGMT_LEVELS <- c("inside_forest", "near_forest", "agroforest", "full_sun")
FARM_LEVELS <- c("ib", "vr", "sa", "kk", "mt", "vi", "yb")

# ── 1. Load data (OTU97 — main analysis) ──────────────────────────────────────

ps_ITS1_otu97_fungi_biosamples <- readRDS(
        here("results", "rds", "ps_ITS1_otu97_fungi_biosamples.rds")
)

cat(sprintf("Loaded (OTU97): %d taxa × %d samples\n",
...
# tax_glom() collapses OTUs to Phylum; transform_sample_counts() computes RA;
# psmelt() converts to long data.frame for ggplot.

fig_long <- ps_ITS1_otu97_fungi_biosamples |>
...
# ── 4. Colour palette (Set2-based, earthy tones distinct from kelly/bacteria) ──
# RColorBrewer::brewer.pal(8, "Set2") gives 8 pastel-earthy colours;
# colorRampPalette() interpolates to however many named phyla exist.

named_phyla <- setdiff(phylum_factor_levels, c("Other", "p__Unassigned"))
n_named     <- length(named_phyla)
base_pal <- RColorBrewer::brewer.pal(8, "Set2")[-c(4, 5, 8)]   # drop pink, green and grey (saved for Unassigned and Other)
phylum_pal  <- colorRampPalette(base_pal)(n_named)
phylum_colors <- c(
        setNames(phylum_pal, named_phyla),
        "p__Unassigned" = "#E68FAC",   # same soft pink as in 07aa_ (kelly palette)
        "Other"         = "grey80"
)
...
figA <- ggplot(figA_tbl, aes(x = farm_id, y = Abundance, fill = Phylum_plot)) +
        ...
        labs(
                ...
                x        = NULL,
                title    = "ITS1 fungal community — farm-level averages (OTU97)",
                subtitle = sprintf(
...
ggsave(
        here("results", "figures", "06b_ITS1_barplot_phylum_by_farm_mean.png"),
        plot = figA, width = 12, height = 8, dpi = 300
)
cat("Saved: 06b_ITS1_barplot_phylum_by_farm_mean.png\n")
...
figB <- ggplot(figB_tbl, aes(x = tree_id, y = Abundance, fill = Phylum_plot)) +
        ...
        labs(
                y        = "Relative abundance",
                x        = NULL,
                title    = "ITS1 fungal community — tree-level averages (OTU97)",
                subtitle = sprintf(
...
ggsave(
        here("results", "figures", "06b_ITS1_barplot_phylum_by_farm_tree_level.png"),
        plot = figB, width = 28, height = 8, dpi = 300
)
cat("Saved: 06b_ITS1_barplot_phylum_by_farm_tree_level.png\n")
...
figC <- ggplot(figC_tbl, aes(x = flower_id, y = Abundance, fill = Phylum_plot)) +
        ...
        labs(
                y        = "Relative abundance",
                x        = NULL,
                title    = "ITS1 fungal community — flower level (OTU97)",
                subtitle = sprintf(
...
ggsave(
        here("results", "figures", "06b_ITS1_barplot_phylum_by_farm_flower_level.png"),
        plot = figC, width = 36, height = 8, dpi = 300
)
cat("Saved: 06b_ITS1_barplot_phylum_by_farm_flower_level.png\n")
