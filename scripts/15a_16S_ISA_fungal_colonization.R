## ============================================================================
## 15a — Indicator Species Analysis (ISA) — 16S Bacteria (CFM)
...
## Output:
##   results/figures/15a_ISA_fungal_colonization.png
##   results/tables/15a_ISA_fungal_colonization.csv
## ============================================================================
...
ps_col <- prune_samples(keep, ps_16S)
...
fig <- (p_high / p_low) +
        plot_layout(heights = c(max(2, n_high), max(2, n_low)),
                    guides  = "collect") +
        plot_annotation(
                title   = "ISA: Fungal colonization footprint — 16S bacteria  (unbagged flowers)",
                caption = sprintf(
...
ggsave(
        here("results", "figures", "15a_ISA_fungal_colonization.png"),
        plot   = fig,
        width  = 12,
        height = max(6, (n_high + n_low) * 0.6 + 3),
        dpi    = 300
)
cat("  Saved: 15a_ISA_fungal_colonization.png\n")
...
if (nrow(hits_bvu) > 0) {
        hits_bvu |>
                ...
                readr::write_csv(here("results", "tables",
                                      "15a_ISA_fungal_colonization.csv"))
        cat("  Saved: 15a_ISA_fungal_colonization.csv\n")
}

cat(sprintf("\n15a done. %d indicators found.\n", nrow(hits_bvu)))
