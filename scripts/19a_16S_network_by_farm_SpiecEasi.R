...
##   results/figures/19a_network_panels.png
...
N_PERM             <- 100     # permutations for netCompare; increase to ≥1000 for publication
...
# Extract signed adjacency from a NetComi netConstruct object → igraph.
# Edge weight = |partial correlation|; edge sign attribute used for colour.
net_to_igraph <- function(net_raw, which = 1) {
        ...
        # assign sign: adj[el] uses 2-column integer matrix indexing
...
# Nodes coloured by phylum; hub nodes (if any) get a bold black border ring.
# Hub taxa are labelled with genus (or higher rank fallback).
# Edge colour: warm = positive, cool = negative partial correlation.
...
title_str   <- "16S Bacteria — co-occurrence networks by farm (bagged vs unbagged)"
caption_str <- sprintf(
